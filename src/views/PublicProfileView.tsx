...
export default function PublicProfileView() {
  ...
  const [chatHistory, setChatHistory] = useState<{ role: 'user' | 'model'; parts: { text: string }[] }[]>([
    {
      role: 'model',
      parts: [{ text: "Hello. I am the digital representative of Zhang San. I have access to his complete resume, GitHub repositories, and architectural design documents up to the current date. How can I assist you in evaluating his qualifications?" }]
    }
  ]);
      ...
      <header className="flex-shrink-0 flex flex-col md:flex-row items-center md:items-start gap-8 p-8 border border-outline-variant/20 rounded-3xl bg-surface-container-lowest/50 backdrop-blur-sm shadow-xl">
        ...
        <div className="flex flex-col items-center md:items-start flex-grow text-center md:text-left gap-2">
          <div className="flex items-center gap-3">
            <h1 className="text-3xl font-black text-on-surface tracking-tight">{INITIAL_PROFILE.name}</h1>
          ...
          <p className="text-sm text-on-surface-variant mt-2 max-w-2xl leading-relaxed opacity-70">
            Welcome to my interactive profile. This localized AI agent is trained on my professional history, project logs, and technical documentation. Feel free to query my experience.
          </p>
          
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 mt-4">
            {['Kubernetes', 'AWS / GCP', 'Go & Rust'].map((tag, i) => (
      ...
      <section className="flex-grow flex flex-col border border-outline-variant/20 rounded-3xl bg-surface-container-low overflow-hidden relative shadow-[0_20px_50px_rgba(0,0,0,0.3)] min-h-0">
        <div className="flex-shrink-0 flex items-center justify-between px-6 py-4 border-b border-outline-variant/20 bg-surface-container backdrop-blur-md">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-on-surface-variant">Interactive Session Logging</span>
          </div>
          <button 
        ...
        <div className="flex-grow p-8 flex flex-col gap-8 overflow-y-auto scrollbar-hide">
            ...
            >
              <div className="flex items-center gap-3 mb-1">
                <span className={cn("text-[9px] font-black uppercase tracking-widest", chat.role === 'user' ? "text-primary" : "text-secondary")}>
                  {chat.role === 'user' ? 'Visitor' : 'Zhang San [Agent]'}
                </span>
                <span className="text-[9px] font-mono text-outline opacity-50">Local Time: 14:02 EST</span>
              </div>
              <div className={cn(
          ...
          {isChatting && (
            <div className="flex flex-col gap-2 self-start items-start border-l-2 border-secondary/20 pl-6 animate-pulse">
        ...
        <div className="flex-shrink-0 p-6 bg-surface-container border-t border-outline-variant/20">
          <div className="flex items-end gap-3 bg-surface-container-low border border-outline-variant/30 rounded-2xl p-2 focus-within:border-primary/50 transition-all shadow-inner">
            ...
            <button 
              onClick={() => handleChat()}
              className="flex-shrink-0 w-11 h-11 rounded-xl bg-primary text-on-primary flex items-center justify-center hover:brightness-110 transition-all shadow-lg"
            >
              <Send size={18} />
            </button>
          </div>
          <div className="mt-3 text-center">
            <span className="text-[9px] font-black uppercase tracking-[0.2em] text-outline opacity-40">AI responses may vary. Verified via cryptographic signature.</span>
          </div>
        </div>
      </section>
    </div>
